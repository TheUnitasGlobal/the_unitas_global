**Codex 제14장 3단계 -- 강제 티어 "SONNET 5 / HIGH"는 `test-results/stage3/latest.json`을 읽는 이 에이전트에 귀속된다(세션을 `claude --model sonnet`, effort high로 연다). 데몬(`web/scripts/idle-sensor-daemon.mjs`)은 모델 무관·토큰 0이다.**
...
- **에이전트는 창립자에게 보내는 첫 응답의 첫 줄에 그 브리핑 한 줄을 그대로 선제 보고한다.** 되묻지 않는다(제4장).
...
- **브리핑이 실패(fail)를 말하면** 아래 4단계로 kind별 분류만 보고하고, **창립자 지시 없이 전경에서 전수 E2E를 재실행하지 않는다**(제14장 1단계). 재검증은 데몬이 다음 유휴 창에 한다.
...
5. **전경에서 3엔진 전수를 다시 돌리지 않는다**(제14장 1단계가 금지). 재검증은 데몬이 다음 유휴 창에 한다 -- 새 `BUILD_ID@HEAD`는 자동으로 스윕 대상이 된다.
