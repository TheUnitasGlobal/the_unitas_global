...
- ~~nav 좁은 뷰포트 너비~~ — **2026-09-14 정정: nav는 WCAG 2.5.8 AA에 적합하다.**
  처음엔 `Charge Coins`(화면 12×24px)를 너비 미달로 적었으나 그것은 규칙의 절반이었다.
  2.5.8 AA의 **간격 예외**(24px 지름 원이 다른 타깃 박스·다른 작은 타깃의 원과 겹치지
  않으면 적합)까지 계산하면 **Pixel 7 위반 0 · iPhone 12 위반 0**. 디자인 변경 불필요.
  판정은 `rev25-crossplatform.spec.js`의 회귀 테스트로 고정했다.

### §4.1 스킵 감사 (후속 구간)

**스킵은 측정되지 않은 주장이다.** 런타임 조건부 스킵 4곳 중 `rev21-footer-links`의 F-2가
**세 엔진에서 매번 스킵**되고 있었다 — 출하된 프리하이드레이션 부트스트랩이 브라우저에서
한 번도 검증되지 않았다는 뜻이다. 원인 두 겹: ① SW가 `_next/static`을 서빙해 `page.route`에
닿지 않음 ② `reload()`에서는 Next 불변 청크가 HTTP 캐시에서 오고 캐시 히트도 라우팅 불가.
**한 번도 로드한 적 없는 페이지의 first load**로 바꾸니 약 3.9초의 진짜 프리하이드레이션 창이
열렸고, 그 안의 클릭이 파킹→미러→모달 개방까지 완주한다. 세 엔진 전부 통과.
자세한 수치는 FINAL_REPORT §8.
