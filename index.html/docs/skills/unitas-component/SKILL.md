...
**No simulated data, ever** (제17장). Filling a gap with a plausible deterministic number is a fail-OPEN; `web/__tests__/square/failOpenRegression.test.ts` statically bans 11 attribute names and 12 exports from returning. `web/lib/square/pulse.ts` survives for SSR-vs-first-frame parity only — never to invent rows, counts or volumes.
...
⚠ A coin-gated module needs a **new timestamped file** under `supabase/migrations/` — never an edit to an existing one, and never apply it yourself (제17장: a live DB mutation is the one thing 제14장 초자동화 자율 승인 does **not** cover — it still needs the founder's explicit `ok`).
