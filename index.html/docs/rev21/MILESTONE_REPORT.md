...
## SEO-SEZNAM — Seznam.cz 소유권 인증 각인 (2026-09-13, 창립자 지령)

**5번째 콘솔.** Seznam.cz는 체코 시장 검색 엔진이며, 네이버와 마찬가지로 Next
`Verification` 타입에 전용 필드가 없어 `verification.other`에 리터럴 meta 이름
`seznam-wmt`으로 각인했다. 토큰 정본은 `lib/seo/routes.ts`의 `SEZNAM_SITE_VERIFICATION`.

**`SeznamBot`은 `NAMED_CRAWLERS`에 넣지 않았다 (얀덱스와 다른 판단):**
`YandexBot` 추가의 근거는 **`ru`가 지원 20로케일에 있다**는 것이었다. 체코어 `cs`는 없다.
명명 블록은 `*` 블록을 **대체**하므로 39줄 payload를 통째로 복제해 영구 동기화해야 하는데,
SeznamBot은 이미 `*` 블록으로 동일 권한을 받고 있다 — **얻는 것 없이 유지 부담만 는다.**
소유권 확인(콘솔에게 하는 말)과 크롤 허용(크롤러에게 하는 말)은 별개이며, 후자는 이미 충족.

**전사 오류 방어:** 이 토큰은 32자 대소문자 혼합 영숫자로 **`I`/`l`/`1` 유사 문자를 포함**한다.
슬립이 나도 `tsc`·`build`·배포가 전부 EXIT 0으로 통과하고 콘솔 거부 시점에야 드러나므로,
형식 검사(`^[A-Za-z0-9]{32}$`)에 더해 **지령 원문 ↔ 빌드 산출물 바이트 비교**를 별도 수행 —
`byte-identical: True`.
...
| --- | --- |
| `tsc --noEmit` | **EXIT 0** |
| `vitest run` | **76 파일 / 1151 테스트 통과** (1150 → +1) |
| `next build` | **EXIT 0** |
| 빌드 산출물 | 사전렌더 **421 HTML 전수 — 4개 태그 각 421/421**, 누락 0, 문서당 각 1개 |

**라이브 실측 (배포 후 HTTPS 실호출):**

| 항목 | 실측값 |
| --- | --- |
| 커밋 | `a4628b4` → origin/main |
| 라이브 `gitCommit` | `a4628b481b93078c…` — 푸시 커밋과 일치 |
| 라이브 `buildFingerprint` | `632c59de6817b47c…` — 로컬 빌드와 일치 |
| 20로케일 루트 | **20/20 HTTP 200 · 구글 20/20 · 네이버 20/20 · 얀덱스 20/20 · Seznam 20/20** |
| 회귀 | `/sitemap.xml` 200 · **340 URL** · `/robots.txt` 200 · **7블록** — 무손상 |
...
| 빙 | 코드 없음 (GSC 연동 상속) | ✓ | **Success / 340 발견** (자동 Import) |
| 네이버 | `other['naver-site-verification']` | ✓ | 등록됨 (처리 중) |
| 얀덱스 | `verification.yandex` | ✓ (Owner, 토큰 일치 실측) | **Indexing 탭 확인 필요** |
| Seznam | `other['seznam-wmt']` | **태그 라이브 — [Ověřit doménu] 클릭 대기** | — |

**정직 표기:** Seznam은 태그가 라이브에 올라갔을 뿐 **소유권은 아직 성립하지 않았다** —
...
## SEO-INDEXNOW — IndexNow 실시간 색인 프로토콜 구축 및 340 URL 송출 완료 (2026-09-13, 창립자 지령)

**결과: 340 URL을 빙·얀덱스·Seznam·네이버에 동시 송출 완료.** 크롤러를 기다리는 대신
변경을 밀어넣는 체계가 가동됐다. 구글은 IndexNow 미참여이므로 기존 사이트맵 경로 그대로다.

**하달 스크립트를 문자 그대로 실행하지 않은 이유 (결함 3건):**

| # | 하달된 형태 | 실제 결과 |
| --- | --- | --- |
| 1 | 키 `unitas999indexnowkey`를 `unitas-indexnow-key.txt`에 기록 | **파일명과 키가 불일치.** 루트 호스팅(Option 1)은 파일명이 정확히 `<키>.txt`여야 하며, 불일치 시 **모든 제출이 403**. → `public/unitas999indexnowkey.txt` |
| 2 | `Set-Content -Encoding utf8` | PS 5.1은 **BOM을 붙인다** → 본문 ≠ 키 → 검증 실패. → BOM·후행개행 없이 **정확히 20바이트**, 테스트가 `.trim()` **없이** 바이트 단정(트리밍이 이 결함을 숨기므로) |
| 3 | 키 파일 생성이 전부 | **URL을 제출하는 코드가 전혀 없었다.** 커밋 메시지는 "프로토콜 통합"이라 선언했으나 키 파일만으로는 아무 일도 일어나지 않는다 → 실제 프로토콜 모듈 구현 |

키 **값**은 유지했다 — 20자 `[a-z0-9]`로 규격(8~128자, `[a-zA-Z0-9-]`) 충족. 공개 URL로
서빙되므로 비밀이 아니라 통제권 증명이며 예측 가능성은 무관하다.

**구축물:** `lib/seo/indexnow.ts`(순수 프로토콜: 키·엔드포인트·페이로드·URL 파티셔닝) ·
`scripts/indexnow-submit.mjs`(송출) · `__tests__/seo/indexnow.test.ts`(회귀 12건) ·
`npm run seo:indexnow`. 엔드포인트는 **공유 엔드포인트 `https://api.indexnow.org/indexnow`
하나**로 전 참여 엔진에 전파된다 — 엔진별 팬아웃 불필요.

**설계 원칙 3가지:**
1. **빌드가 아니라 배포 *후* 실행.** 미배포 콘텐츠의 URL을 밀면 엔진이 즉시 크롤해
   **옛 페이지를 캐싱**한다 — 제출 안 하느니 못하다.
2. **송출 전 라이브 증명.** 키 파일이 실제 서빙 중인지 네트워크로 확인한 뒤에만 제출하고,
   **라이브 사이트맵이 광고하는 것만** 보낸다(콘솔에 준 집합과 드리프트 불가).
3. **키 중복 선언 없음.** `public/`에서 **파일명이 곧 자기 내용인 파일**을 찾아 키를 얻는다 —
   규격이 요구하는 바로 그 불변식이라 탐색과 검증이 동일 행위다.

**실측 (4대 게이트 전부 EXIT 0):** `tsc` 0 · vitest **77파일 / 1163 테스트**(1151 → +12) ·
`build` 0 · 배포 전 dry-run이 `HTTP 404 ... deploy first`로 **정상 거부(EXIT 1)**.

**라이브 (커밋 `22c5862`, 배포 후):**

| 항목 | 실측값 |
| --- | --- |
| 라이브 `gitCommit` | `22c5862d2f4bbfab…` — 푸시 커밋과 일치 |
| 키 파일 | `HTTP 200 · text/plain · 20 bytes` · 본문 `unitas999indexnowkey` · **BOM 0** |
| robots 차단 여부 | **차단 없음** (39개 Disallow 규칙 전수 판정) |
| verification 태그 회귀 | 루트 페이지 **4개 전부 정상** |
| 사이트맵 / robots | 340 URL / 7블록 무손상 |
| **송출** | **340 URL · HTTP 200 · EXIT 0** |

**첫 실행에서 겪은 것 (재조사 금지):** 첫 송출은 **HTTP 403
`SiteVerificationNotCompleted`** 로 거부됐다. 코드 결함이 아니라 **엔진이 새 키 파일을
비동기로 가져가 검증하는 동안의 정상 지연**이다. 사이트를 전혀 바꾸지 않은 채 몇 분 뒤
재시도하니 **4개 엔드포인트 전부 수락**(api.indexnow.org 200 · bing 200 · yandex 202
`{"success":true}` · seznam 200). 스크립트가 이 403을 식별해 "transient, 몇 분 뒤 재시도"
안내를 출력하도록 보강했다.

**정직 표기 — 내 쪽 결함 2건:**
1. 송출 스크립트 초판이 fetch 진행 중 `process.exit()`를 호출해 Windows에서 libuv 어서션
   크래시(`UV_HANDLE_CLOSING`)를 냈고 종료 코드가 `1`이 아닌 **0xC0000409**로 나왔다.
   메시지는 맞았으나 종료 코드를 읽는 쪽엔 "크래시"로 보인다. 예외를 던지고
   `process.exitCode`를 설정하는 방식으로 교정. **실제로 실행해 봤기에 잡혔다.**
2. robots 차단 판정용 셸 매처가 `/*/apex` 같은 규칙에서 접두사를 `/`로 잘라
   **모든 경로에 매치**시켜 "키 파일 차단됨"이라는 거짓 양성을 냈다. 규격대로 재구현하고
   기지 케이스 6건으로 검증한 뒤 **차단 없음**을 확인. **교훈: robots 매처는 `*`를
   `.*`로 치환한 정규식으로 만들고, 반드시 기지 케이스로 자기 검증할 것.**
3. 부수: `public/`에 파일이 늘어 소유권 매니페스트가 **12파일 `632c59de…` → 13파일
   `4af93cbd…`** 로 이동했다. 정상이며 드리프트가 아니다.

**운영 지침:** 앞으로 **프로덕션 배포 직후 `npm run seo:indexnow` 1회** 실행하면 변경이
빙·얀덱스·Seznam·네이버에 즉시 전파된다. 구글은 이 경로를 쓰지 않으므로 사이트맵 재크롤을
기다린다.
