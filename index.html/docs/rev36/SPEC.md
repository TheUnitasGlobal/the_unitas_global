...
- 훅 컨텍스트: 첫 줄 `[Stage-3 자율 브리핑 · Codex 제13장 3단계 · REV-36 M2]`, 둘째 줄 위 1줄, 이어서 details(`- ` 목록), 마지막 줄 `에이전트 지시: 창립자에게 보내는 첫 응답의 첫 줄에 위 브리핑 한 줄을 그대로 선제 보고하라. 되묻지 않는다(제4장 제로 핸즈). 실패가 있으면 READER.md 4단계로 분류만 보고하고 창립자 지시 없이 전수 E2E를 재실행하지 않는다.`
