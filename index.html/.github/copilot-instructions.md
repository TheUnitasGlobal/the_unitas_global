...
- **v23.0 신규 구조 (2026-09-13 · 제21~24장 재편, 구 제21장 멀티 모델 페일오버 독트린 삭제):** **제22장 스마트 로우메모리 아머(Soft Limit 50k)** — 메인 실행 에이전트를 제외한 모든 보조 서브에이전트·백그라운드 프로세스의 상시 가동 엄금(온디맨드 소집만 허용), 누적 토큰 50k 도달 시 자율 컨텍스트 압축으로 과거 로그를 폐기하고 설계 정본만 유지. **제23장 스마트 휴먼-인-더-루프 세이프티 브레이크(Hard Limit 70k)** — 70k 초과 또는 대규모 마일스톤 완결 시에만 발동, 정지 시 중간 검증 결과·주요 수치를 `docs/rev21/MILESTONE_REPORT.md`에 강제 기록(Flush)하고 지정된 브레이크 문구를 정확히 출력한 뒤 창립자 승인(`/clear` 후 `next`) 전까지 절대 속행 금지, 창립자 신호 `next`(다음 마일스톤 착수)와 `ok`(승인 확정) 엄격 분리. **제24장 Fail-Closed 무결성 검증·배포·최종 완결 종합 보고** — `typecheck`·`vitest`·`build`가 전부 EXIT 0이 아니면 커밋·푸시·프로덕션 배포 전면 금지, `sync-codex` 드리프트 게이트(prebuild 배선)·ownership 지문·Sovereign Shield 자가치유 바운더리 상시 가동, 측정하지 않은 상태를 완료로 보고하는 행위 영구 금지, 최종 시점에 [최종 완결 종합 보고서]를 터미널과 로컬 파일에 집대성.
