...
> 이 전문(全文)은 정본으로서 다음에 동일하게 각인·동기화되어 있다: 루트 `CLAUDE.md`, `index.html/CLAUDE.md`(이 파일), `index.html/THE_UNITAS_GLOBAL_MASTER_ARCHIVE.md`, `index.html/.roo/rules/unitas-constitution.md`. 나머지 엔진 파일(`.github/copilot-instructions.md`, `.continue/config.yaml`, `.github/agents/*.agent.md`, `.aider.conf.yml`)에는 요약본 + 이 정본에 대한 포인터가 들어간다. 헌법 개정 시 위 목록 전체를 같은 커밋에서 손으로 동기화한다.

> **[유니타스 경영 지침서 (통합)] — THE UNITAS GLOBAL OÜ Master Operational Codex** (2026-08-31 창립자 지시로 전면 통합 각인. 아래 제1~6장이 §0의 정본 본문이다. 이 파일 하단의 "Low-Memory Armor", "Automated git sync + lightweight self-healing gate (Stop hook)", "Output language & standing approval protocol", "Session hygiene / token efficiency" 절은 제4장의 저장소 전용 구현 상세로서 계속 유효하다.)
...
- 제로 타협 원칙: 저사양 하드웨어를 이유로 고성능 툴, WebGL 3D, Playwright 등 핵심 기능을 일체 비활성화하거나 축소하지 않음. (구체 규칙: 아래 "Low-Memory Armor" 절.)
- 로우메모리 아머: 유휴 백그라운드 파일 워처 철저 차단으로 PC 런타임을 보호하고, 필요 순간 100% 온디맨드 초고속 호출.
- Fail-Closed 게이트: 모든 작업 직후 반드시 사전 예방적 무결성 검증(npm --prefix web run typecheck 및 build)을 강제 수행하여 에러 0건(EXIT 0)을 증명한 뒤 Vercel 프로덕션 배포 및 Git 동기화 완결. (구현: 아래 "Automated git sync + lightweight self-healing gate (Stop hook)" 절.)
...
- 창립자 전용 순차 검증 풀코스·대화 통제 독트린·자율 승인 분기(Smart Auto-Accept)·Korean-First 출력·토큰 다이어트 원칙은 각각 제5·6장 및 이 파일 하단 절에서 구체화된다.
