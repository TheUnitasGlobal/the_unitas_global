...
- **토큰 다이어트 원칙 (Token Diet Principle, owner instruction 2026-08-29):** 주요 공정(기능 단위 완료·배포 체크포인트 안착·무관한 여러 작업의 경계)이 끝나면 에이전트는 창립자 지시를 기다리지 않고 자율적으로 `/compact`(요약 유지) 또는 `/clear`(전체 초기화)를 수행해 150k 컨텍스트 한계 안에서 세션을 운용한다. `.claude/settings.json`의 `autoCompactEnabled: true` + `autoCompactWindow: 70000`이 자동 안전망이며, `/clear`는 실제 마일스톤 경계(기능 완료·체크포인트 푸시 직후·여러 무관한 작업 전환)에서 수행한다. 아래 "Session hygiene / token efficiency" 절이 구현 지침이다.
- **제로 타협 에이전트 자율 구동 (Zero-Compromise Autonomous Operation):** 에이전트는 하드웨어·컨텍스트·토큰 제약을 이유로 공정의 완결성을 낮추지 않는다. 탐색→구현→검증(`npm --prefix web run typecheck` + `build`)→배포 전 과정을 자율 완주하며, 중단 지점은 오직 헌법이 명시한 승인 게이트(DB 파괴·파일 강제 삭제·외부 배포·핵심 아키텍처/정책 설계 선택지)뿐이다.
- **정밀 지시 체계 유지 (Precision Directive Discipline):** 창립자의 다항목 지시는 번호·항목 단위로 완전 이행하고, 각 항목의 이행 결과를 한국어로 정확히 보고한다. 누락·부분 이행·미검증 항목을 완료로 보고하지 않으며, 이행 불가·보류 항목은 사유를 명시한다.
...
- **U-AI FREE Phase-1 live web synthesis**: `web/lib/uai/webSynthesis.ts` folds *real* online references into the free (zero-coin, zero-LLM) surface search — keyless, CORS-only Wikipedia/Wikimedia REST calls made client-side, hard 2.5s abort, 24h `localStorage` cache, gated by `NEXT_PUBLIC_UAI_WEB_SYNTHESIS` (`1`/`true` = on). Fail-open: disabled/timeout/network error → `sourced:false` and `analyzeSurface` runs on the query alone, no error surfaced. `heuristics.ts` then runs the **51-doctrine deconstruction** (constitution → 6 scored axes: logic/future/economy/security/sovereign/art) + a blind-spot "redesign vector". No API key, no server route, no cost — the flag is NOT set on Vercel yet, so production currently runs pure-heuristic until `vercel env add NEXT_PUBLIC_UAI_WEB_SYNTHESIS production` (value `1`).
- **U-AI LLM provider**: `web/lib/uai/provider.ts` uses `ANTHROPIC_API_KEY` (Claude) when set, else falls back to `OPENAI_API_KEY`. As of 2026-08-30 Vercel production has `OPENAI_API_KEY` + `SUPABASE_SERVICE_ROLE_KEY` set (the latter also unblocks `/api/account/delete` and the Genesis Memory cache / server-side `brain_grid` writes); `ANTHROPIC_API_KEY` is NOT set — add it via `vercel env add ANTHROPIC_API_KEY production` to switch the engine to Claude with no code change. Migration `20260908000000` was applied to the live DB (`supabase db push` + `migration repair`), so `spend_coins('u-ai', …)` is live. `generateInsight(system, user, maxTokens?)` takes an optional token budget (default 1800; the 6-axis redesign passes `REDESIGN_MAX_TOKENS = 2600`).
