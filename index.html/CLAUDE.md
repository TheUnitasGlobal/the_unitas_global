...
- **Migration `20260908000000_u_ai_genesis_memory.sql` needs manual apply** to the live project (direct SQL + `supabase migration repair --status applied`, NOT `db push` — same convention as 20260902/20260903). Until applied: `spend_coins('u-ai', …)` raises `Unknown module`, and the route degrades every deep request to a cache miss while logging nothing to `brain_grid`.
