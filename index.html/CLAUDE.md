...
- **U-AI deep insight has no auto-refund.** `POST /api/u-ai/insight` requires the client to `spend_coins('u-ai', N)` *before* the call (proof-of-burn = a `module_access_grant` < 120s old). If the Claude generation then fails, the coin stays burned — `credit_coins()` is Stripe-webhook-only + idempotent on a payment-intent id, so there's no clean refund path. Documented, matching the "absolute margin / Micro-Burn" design intent (owner instruction 2026-08-30). See `web/lib/uai/*` + `supabase/migrations/20260908000000_u_ai_genesis_memory.sql`.
