 ...
 */
import { compactNumber, deeperFetchJson, wikidataUrl } from '../deeperFetch';
 ...
 * property; `f1..f6` are the theme's own field labels (see
 * `Rev23.deeper.themes.bigTechPulse` in messages/*.json).
 */
...
export const bigTechPulseAdapter: DeeperAdapter = {
  key: 'bigTechPulse',
  async load(anchor, ctx: DeeperContext, cursor): Promise<DeeperPage> {
    ...
    if (!entity) return EMPTY_DEEPER_PAGE(['wikidata']);
