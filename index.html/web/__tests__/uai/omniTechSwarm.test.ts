...
const ctx: SwarmContext = { locale: 'ko', lang: 'ko' };
/** Samsung Electronics. */
...
async function loadAllPages() {
  const cards = [];
  let cursor: SwarmCursor | null = null;
  for (let guard = 0; guard < 6; guard++) {
    const page = await omniTechSource.load(ORG, ctx, cursor ?? undefined);
...
describe('bigTechPulse adapter -- the data the swarm eats', () => {
  it('fails closed on an anchor with no entity', async () => {
    const page = await omniTechSource.load({ ...ORG, qid: undefined }, ctx);
...
describe('adapter -> swarm, end to end', () => {
  ...
  it('grows a sector at a time as cursor pages land, without moving what is there', async () => {
    const first = await omniTechSource.load(ORG, ctx);
