...
/**
 * The UNITAS Shorts CREATOR PASS (REV-19 follow-up, revived in REV-29 M4):
 * a premium, layered 3D-glass pre-reservation surface (app/unitas-hub.css)
 * that replaces the dead "Upload" tap. The visitor picks a creator handle
 * and reserves a numbered pass; the pass lives on this device and, when
 * signed in, on the account's `user_metadata` (no schema change). Honest by
 * design: the copy says plainly that uploads open with the U-Messenger beta
 * and that no video is stored yet. Opens as a level on the deep modal
 * history stack.
 */
export function ShortsCreatorPass({ open, onClose }: ShortsCreatorPassProps) {
  const t = useTranslations('Rev29.shorts.pass');
          ...
          <div className="u-wl-card-inner">
            ...
            <div className="u-wl-card-body">
              ...
              <div className="u-wl-form">
                ...
                {sealed && !bound && (
                  <p className="u-wl-status" data-tone="muted">
                    {t('guest')}
                  </p>
                )}
              </div>
