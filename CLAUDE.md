...
> 이 전문(全文)은 정본으로서 다음에 동일하게 각인·동기화되어 있다: `CLAUDE.md`(이 파일), `THE_UNITAS_GLOBAL_MASTER_ARCHIVE.md`, `.roo/rules/unitas-constitution.md`. 나머지 엔진 파일(`.github/copilot-instructions.md`, `.continue/config.yaml`, `.github/agents/*.agent.md`, `.aider.conf.yml`)에는 요약본 + 이 정본에 대한 포인터가 들어간다. 헌법 개정 시 위 목록 전체를 같은 커밋에서 손으로 동기화한다.
...
- **이 저장소가 정본이다.** `origin` → `github.com/TheUnitasGlobal/the_unitas_global` → Vercel 프로덕션 자동배포(`www.theunitas.global` / `theunitas.global`). **`git push origin main`이 곧 프로덕션 배포다** — 페일클로즈드 게이트 통과 후에만 push한다.
- **운영 루트:** `web/` — Next.js 14 (App Router) + Supabase 앱. 아래 모든 규칙의 경로 기준. 레거시 정적 사이트는 저장소 루트(`index.html`, `assets/`, `pages/`)이며 read-only.
